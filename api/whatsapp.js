const rateLimitStore = globalThis.__bomberRateLimitWA || (globalThis.__bomberRateLimitWA = {});
const MAX_POSTS = 10;
...
export default async function handler(req, res) {
  ...
  // upstream WhatsApp API expects number WITHOUT the + sign
  const numberNoPlus = text.replace("+", "");

  try {
    const upstreamUrl = `http://45.134.39.212:4065/api/text?number=${encodeURIComponent(numberNoPlus)}`;
